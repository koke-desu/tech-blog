...
const PostThumbnail: React.FC<Props> = ({ post }) => {
  return (
    <article className="border border-gray shadow-md rounded-md w-72 aspect-square flex flex-col items-center">
      <div className="w-full aspect-video relative">
        <Image fill src={post.thumbnail} alt={post.title} className="object-contain" />
      </div>
      <h2 className="font-bold text-2xl my-4 line-clamp-2 text-text">{post.title}</h2>
